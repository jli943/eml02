...
class QMultiheadAttention(MultiheadAttention):
    ...
    def __init__(self, embed_dim, num_heads, **kwargs):
        ...
        # Replace nn.Linear with QLinear
        self.q_proj = QLinear(embed_dim, embed_dim) 
        self.k_proj = QLinear(embed_dim, embed_dim)
        self.v_proj = QLinear(embed_dim, embed_dim)
        self.out_proj = QLinear(embed_dim, embed_dim)
        print("calling QMultiheadAttention")
        
    def forward(self, query, key, value, num_bits, num_grad_bits, **kwargs):

        # Quantize projections
        q = self.q_proj(query, num_bits, num_grad_bits)
        k = self.k_proj(key, num_bits, num_grad_bits)
        v = self.v_proj(value, num_bits, num_grad_bits)
        
        # Attention
        attn_weights, attn = super().forward(q, k, v, **kwargs)
        
        # Quantize output projection
        attn = self.out_proj(attn, num_bits, num_grad_bits)
        
        return attn, attn_weights

MultiheadAttention = QMultiheadAttention
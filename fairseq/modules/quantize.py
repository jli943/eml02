...
#         self.num_bits = None  # To be set during forward pass
    
#     def forward(self, input, num_bits, num_grad_bits):

#         if num_bits == 0:
#             return super().forward(input)  # Skip quantization if num_bits is 0

#         self.num_bits = num_bits  # Store for quantization of input in backward pass

#         if self.bias is not None:
#             qbias = quantize(self.bias, num_bits=num_bits)
#         else:
#             qbias = None

#         weight_qparams = calculate_qparams(self.weight, num_bits=num_bits, flatten_dims=(0,))
#         qweight = quantize(self.weight, qparams=weight_qparams)

#         qinput = self.quantize_input(input, num_bits)
#         output = F.linear(qinput, qweight, qbias)
#         output = quantize_grad(output, num_bits=num_grad_bits)
...
class QLinear(nn.Linear):
    ...
    def forward(self, input, num_bits, num_grad_bits=None):
        if num_bits == 0:
        ...
        weight_qparams = calculate_qparams(self.weight, num_bits=num_bits, 
                                           flatten_dims=(1, -1), reduce_dim=None) 
        qweight = quantize(self.weight, qparams=weight_qparams)
        
        qinput = self.quantize_input(input, num_bits)
        output = F.linear(qinput, qweight, qbias)
        if num_grad_bits is not None:
            output = quantize_grad(output, num_bits=num_grad_bits, flatten_dims=(1, -1))
